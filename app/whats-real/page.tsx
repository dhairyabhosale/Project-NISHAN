...
export default function WhatsRealPage() { const { locale } = useLocale(); return <main className="mx-auto max-w-2xl px-4 py-10"><h1 className="text-3xl font-black">{resolve("real.title", {}, locale)}</h1><div className="mt-8 space-y-5">{sections.map(([heading, body]) => <section key={heading} className="rounded-2xl bg-white p-5 shadow-sm"><h2 className="text-xl font-black">{resolve(heading, {}, locale)}</h2><p className="mt-3 leading-relaxed text-neutral-700">{resolve(body, {}, locale)}</p></section>)}</div></main>; }
